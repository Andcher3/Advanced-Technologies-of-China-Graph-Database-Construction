...
# 用于处理列表属性的函数，格式化为 LOAD CSV 可读的字符串
def _format_list_property(data_list: list) -> str:
    """Formats a list of strings into a Neo4j LOAD CSV compatible string representation."""
    if not data_list:
        return ""
    # Ensure items are strings and escape quotes
    safe_items = [str(item).replace('"', '""') for item in data_list if item is not None]
    # Neo4j LOAD CSV handles lists within square brackets
    return f'[{",".join(f"{item}" for item in safe_items)}]'  # LOAD CSV usually doesn't need quotes inside [] if no comma/quote issues, but safer to add them


def _sanitize_label(label_str: str) -> str:
